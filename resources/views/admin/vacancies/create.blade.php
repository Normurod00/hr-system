...
@section('header', 'Создать вакансию')
...
.tip-item__icon.red { background: rgba(239, 68, 68, 0.1); color: #dc2626; }
.tip-item__icon.yellow { background: rgba(245, 158, 11, 0.1); color: #d97706; }
.tip-item__icon.blue { background: rgba(59, 130, 246, 0.1); color: #2563eb; }
...
<div class="page-intro">
    ...
    <div class="page-intro__content">
        ...
        <p>Заполните информацию о вакансии. AI-робот автоматически проанализирует резюме кандидатов и рассчитает соответствие вашим требованиям.</p>
    </div>
</div>

<div class="row">
    <div class="col-lg-8">
        <form action="{{ route('admin.vacancies.store') }}" method="POST" id="vacancyForm">
            @csrf
                ...
                <div class="form-section__body">
                    ...
                    <div class="mb-0">
                        ...
                        <textarea class="form-control @error('description') is-invalid @enderror"
                                  ...
                                  name="description"
                                  rows="6"
                                  placeholder="Опишите обязанности, требования, условия работы, преимущества компании..."
                ...
                <div class="form-section__body">
                    <label class="form-switch-custom">
                        ...
                        <span class="form-switch-custom__label">
                            <span class="form-switch-custom__title">Опубликовать вакансию</span>
                            <span class="form-switch-custom__desc">Вакансия будет видна кандидатам на сайте</span>
                        </span>
                    </label>
                </div>
            </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="tips-card">
            <div class="tips-card__header">
                ...
                <span>Советы по созданию</span>
            </div>
            <div class="tips-card__body">
                <div class="tip-item">
                    <div class="tip-item__icon red">
                        <i class="bi bi-check-circle-fill"></i>
                    </div>
                    <div class="tip-item__content">
                        <strong>Must-have навыки</strong>
                        <p>Укажите ключевые технологии и навыки, без которых кандидат не подходит. Они имеют наибольший вес в match score.</p>
                    </div>
                </div>
                <div class="tip-item">
                    <div class="tip-item__icon yellow">
                        <i class="bi bi-star-fill"></i>
                    </div>
                    <div class="tip-item__content">
                        <strong>Nice-to-have навыки</strong>
                        <p>Дополнительные навыки, которые будут плюсом. Повышают рейтинг, но не критичны.</p>
                    </div>
                </div>
                <div class="tip-item">
                    <div class="tip-item__icon blue">
                        <i class="bi bi-robot"></i>
                    </div>
                    <div class="tip-item__content">
                        <strong>AI-анализ</strong>
                        <p>Робот автоматически проанализирует резюме и рассчитает % соответствия каждого кандидата.</p>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initSkillsDropdown(config) {
        ...
        function renderTags() {
            tagsContainer.innerHTML = skills.map(skill => `
                <span class="skill-tag ${tagClass}">
                    ${skill}
                    <span class="skill-tag__remove" data-skill="${skill}">
                        <i class="bi bi-x"></i>
                    </span>
                </span>
            `).join('');

            tagsContainer.querySelectorAll('.skill-tag__remove').forEach(btn => {
        ...
        document.addEventListener('click', function(e) {
            if (!menu.contains(e.target) && !searchInput.contains(e.target) && !toggle.contains(e.target)) {
                closeMenu();
            }
        });
    ...
    initSkillsDropdown({
        hiddenInputId: 'must_have_skills',
        searchInputId: 'must_have_skills_search',
        tagsContainerId: 'mustHaveTags',
    ...
    initSkillsDropdown({
        hiddenInputId: 'nice_to_have_skills',
        searchInputId: 'nice_to_have_skills_search',
        tagsContainerId: 'niceToHaveTags',
